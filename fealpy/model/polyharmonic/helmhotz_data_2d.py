...
class HelmholtzData2d():
    """
    2D Helmholtz equation data generator with analytical solutions and boundary conditions:
    
        -Δu(x,y) - k²u(x,y) = f(x,y),   (x,y) ∈ [-0.5,0.5]x[-0.5,0.5]
              ∂u/∂n + iκu = g(x,y)      on ∂Ω
    
    with the exact solution:
        u(x, y) = [cos(kr) - c·J₀(kr)] / k ,
    where:
        r = √(x²+y²), 
        c = (cosk + i·sink)/(J₀(k) + i·J₁(k)),
        k is wave.
        J₀(kr) is the Bessel function of the first kind of order 0 at points k*r.
    
    source: f(x,y) = sin(kr)/r.
    """
    
    def set_wave(self, k=1.0):
