...
class DiffusionReactionPINNModel(ComputationalModel):
    ...
    def pde_residual(self, p: TensorLike) -> TensorLike:
        ...
        u = self.net(p)
        f = self.pde.source(p)
        reaction = (self.pde.reaction_coef(p) * u).flatten()
    ...
    def show(self):
        ...
        if self.gd <= 2:
            ...
            if self.gd == 1:
                # 绘制真实解和预测解
                plt.plot(node, u_fem, 'b-', linewidth=3, label='Exact Solution')
                plt.plot(node, u_pred, 'g--', linewidth=2, label='PINN Prediction')
                plt.plot(node, u_pred-u_fem, 'r-', linewidth=2, label='Error: PINN-Exact')
