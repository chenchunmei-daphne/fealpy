...
net_2= nn.Sequential(nn.Linear(2, NN, dtype=torch.float64),nn.Tanh(),
                      nn.Linear(NN, NN//2, dtype=torch.float64), nn.Tanh(),
                      nn.Linear(NN//2, NN//4, dtype=torch.float64), nn.Tanh(),
                      nn.Linear(NN//4, 1, dtype=torch.float64))
# 网络实例化
...
fig_mesh = plot_mesh(mesh=mesh, solution=solution, s1=s_1, s2=s_2)
fig_error = plot_error(error_real, error_imag, error)
plt.show()